...
const StartPage = () => {
  ...
  const [{ user, allSongs, allArtists, allAlbums, songType }, dispatch] =
    useStateValue();

  const myPlaylistSetter = () => {
    dispatch({
      type: actionType.SET_SONG_TYPE,
      songType: "playlist",
    });
  };

  const allSongsSetter = () => {
    dispatch({
      type: actionType.SET_SONG_TYPE,
      songType: null,
    });
    ...
    <div className="w-full p-4 flex items-center justify-center flex-col ">
      ...
      {user && user.user.playlist.length > 0 && (
        <div className="relative w-full my-4 p-4 gap-4 pt-32 pb-14  border-4 border-gray-900 shadow-lg bg-gray-100 rounded-md ">
          ...
          <div onClick={myPlaylistSetter}>
            <SongContainerMain data={user.user.playlist} />
          </div>
        </div>
      )}
