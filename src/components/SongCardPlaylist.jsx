...
const SongCardMain = ({ data, index, type }) => {
  ...
  const deleteUserPlaylist = (userId, song) => {
    // console.log(user.user, [...user.user.playlist, song], song._id);
    let deleteIndex;
    setSong(false);

    var filtered = user.user.playlist.filter(function (value, index, arr) {
      if (value._id === song._id) {
        return null;
      }
      return value;
    });
    changingUserPlaylist(userId, filtered).then((res) => {
      if (res) {
        dispatch({
          type: actionType.SET_USER,
          user: res.data,
      ...
      {firstTime && song && (
        <div className="w-full absolute bottom-2 left-40 flex items-center justify-between px-4">
          <motion.i
            ...
            onClick={() => {
              setIsUserPlaylistUpdated(true);
              deleteUserPlaylist(user.user._id, data);
            }}
          >
