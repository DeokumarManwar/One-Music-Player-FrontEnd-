...
const MusicPlayer = () => {
  const [{ allSongs, songIndex, isSongPlaying, songType, user }, dispatch] =
    useStateValue();
  const [minimize, setMinimize] = useState(null);
  const [isPlaylist, setIsPlaylist] = useState(false);
  // const [songArray, setsongArray] = useState([]);
  let songArray;
  console.log(songType, songArray);

  if (songType === "playlist") {
    songArray = user.user.playlist;
  } else {
    songArray = allSongs;
  }
  ...
  return (
    <React.Fragment>
      {minimize === true && isSongPlaying && (
        <img
          alt=""
          src={songArray[songArray.index || songIndex]?.imageURL}
      ...
      {isSongPlaying && (
        ...
        >
          <div className={`w-full items-center gap-3 p-4 flex relative`}>
            <img
              alt=""
              src={songArray[songIndex]?.imageURL}
              className="w-40 h-20 object-cover rounded-md"
            />
            <div className="flex items-start flex-col">
              <p className="text-xl text-headingColor font-semibold">
                {`${
                  songArray[songIndex]?.name.length > 20
                    ? `${songArray[songIndex]?.name.slice(0, 20)}...`
                    : songArray[songIndex]?.name
                }`}
                {/* {`${allSongs[songIndex]?.name}`} */}
                {"  "}
                <span className="text-base">
                  ({songArray[songIndex]?.album})
                </span>
              </p>
              <p className="text-textColor">
                {songArray[songIndex]?.artist}{" "}
                <span className="taxt-sm text-textColor font-semibold">
                  ({songArray[songIndex]?.category})
                </span>
              </p>
              <motion.i
                whileTap={{ scale: 0.8 }}
            ...
            <div className="flex-1">
              <AudioPlayer
                src={songArray[songIndex]?.songURL}
